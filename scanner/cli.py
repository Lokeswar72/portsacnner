...
def main():
    ...
    scanner = PortScanner(hosts=hosts, ports=ports, timeout=timeout, threads=threads)
    print(f'[*] Starting scan: hosts={len(hosts)} ports={len(ports)} threads={threads} timeout={timeout}s')
